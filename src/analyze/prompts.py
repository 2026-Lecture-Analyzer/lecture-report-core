...
ITEM_GUIDES = {
    ...
    "C3_analogy": (
        ...
        "프로그래밍·DB 강의에서는 SQL 쿼리·코드를 활용해 개념을 보여주는 예시도 "
        "비유·예시로 인정한다(예: '뷰는 가상 테이블입니다. SELECT * FROM v_salary처럼 "
        "원본 테이블 대신 뷰를 조회하면 됩니다'). "
        "코드 실행 결과만 나열하거나 개념 설명 없이 타이핑만 시연하는 것은 인정하지 않는다. "
        "단, 코드의 의도·흐름을 설명하는 것은 C3_code_explanation이 평가하므로 이 항목에서는 제외한다. "
        "근거 인용 시 어떤 개념을 위한 예시인지 알 수 있도록 개념 설명 문장과 예시 문장을 함께 인용해야 한다."
    ),
    "C3_prerequisite": (
        "심화 내용에 진입하기 전에 선행 지식을 점검하거나 상기시키는지 본다. "
        "명시적 점검('다들 ~기억나시죠? 이거 알아야 다음을 이해합니다')은 4~5점. "
        "능동적 점검이 아니어도 '앞에서 배운/이미 아시다시피/지난 시간에 한' 처럼 선행 개념을 "
        "가볍게 상기시키고 심화로 넘어가는 암묵적 연결도 정상으로 보아 3점 이상 인정한다 "
        "(완벽한 점검만 고집하지 말 것). 2점 이하는 선행 상기가 거의/전혀 없이 심화 용어가 "
        "맥락 없이 갑자기 등장하는 경우로 한정한다. "
    ...
    "C5_practice": (
        "이론 설명 후 실습/적용으로 잇는지 본다. 강사가 배운 내용을 직접 실행·시연하거나"
        "(예: 작성한 코드·명령을 바로 실행해 보임), 워밍업·연습 문제를 풀게 하거나, 배운 개념·함수를 "
