...
ITEM_GUIDES = {
    ...
    "C2_structure": (   # TODO(이지선): 고도화
        "강의 내용이 학습자가 이해하기 쉬운 순서로 구성되어 있는지 평가한다. "
        "단순히 오늘 다룰 주제, 기술 용어, 개념명을 나열한 문장은 '목차/주제 예고'에 가까우므로 "
        "그 자체만으로 설명 구조성의 핵심 근거로 인용하지 마라. "
        "다만 도입부에서 목차나 주제를 예고했다는 이유만으로 감점하지는 마라. "
        "감점은 설명 본문 자체가 흐름 없이 항목만 나열되거나, 주제가 자주 바뀌어 학습 흐름을 파악하기 어려울 때 적용한다. "
        "좋은 근거는 개념 설명, 배경 설명, 예시, 시연, 실습, 정리 등이 서로 어떻게 이어지는지 "
        "흐름이 실제 발췌 안에서 확인되는 경우다. "
        "전환 표현(먼저/다음으로/이제/그다음/예를 들어/실제로 해보면 등)이 있어도, "
        "그 뒤에 실제 설명 단계의 연결이 이어지는지 확인하라. "
        "근거 인용 시에는 단순 항목 나열이 아니라 설명 단계 간 연결이나 진행 순서가 드러나는 문장을 선택하라. "
        "전사 특성상 단어가 깨질 수 있으므로, STT 오인식이나 표기 오류를 구조의 결함으로 오해하지 마라. "
        "흐름 일부만 보이면 3점, 설명 단계의 연결이 명확하면 4~5점, "
        "설명 본문이 나열 중심이거나 흐름이 뒤섞이면 1~2점."
    ),
    "C2_emphasis": (  # TODO(이지선): 고도화
        "중요한 개념, 주의점, 실무적으로 중요한 내용을 강사가 명시적으로 강조하거나 반복해서 전달하는지 평가한다. "
        "'중요하다/반드시/꼭/주의해야 한다/핵심은/기억해야 한다/헷갈리기 쉽다/다시 말하면' "
        "같은 중요도 신호가 있거나, 같은 핵심 내용을 다시 짚어 학습자의 주의를 환기하는 경우를 인정한다. "
        "단, 표현 자체만 보고 판단하지 말고, 실제로 무엇이 왜 중요한지 또는 무엇을 주의해야 하는지가 드러나는지 확인하라. "
        "단순한 예시, 개념 설명, 코드 설명, 사용법 설명은 그 자체만으로 핵심 강조 근거가 아니다. "
        "중요도 표시나 반복 강조 없이 일반 설명만 있는 문장은 evidence 로 인용하지 마라. "
        "근거 인용 시에는 무엇이 왜 중요한지 드러나는 문장, 주의점이나 핵심 포인트를 명확히 짚는 문장을 선택하라. "
        "강조 신호가 거의 없으면 1~2점, 일부 핵심을 짚지만 제한적이면 3점, "
        "중요 내용을 여러 차례 명확히 강조하면 4~5점."
    ),
    "C2_summary": (
    ...
    "C5_example": (  # TODO(김예슬): 고도화
        ...
        "직접 연결되고 현업 맥락이 있으면 높게. 예시가 여럿이면 가장 실무적인 것을 근거로. "
        "단순·동떨어진 예시는 낮게."
    ),
    "C5_practice": (  # TODO(김예슬): 고도화
        "이론 설명 후 실제 실습/적용(쿼리 실행·직접 작성·연습 문제)으로 이어지는지 본다. "
        "강사 시연도 인정하되 휴식 공지·UI 안내·단순 이론 설명은 실습이 아니다. "
        "이론↔실습 연결이 자연스러우면 높게."
    ),
}
