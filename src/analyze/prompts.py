...
ITEM_GUIDES = {
    ...
    "C4_transition": (
        "주제 변경·새 챕터 진입·이론에서 실습으로 넘어가는 시점에 전환을 안내하는 "
        "멘트가 있는지 본다. '이제 ~로 넘어가겠습니다 / 다음은 ~를 해보겠습니다 / "
        "~부터 진행합니다'처럼 새 주제 시작을 선언하면 인정하고, '시험 범위라서 / "
        "실무에서 쓰므로' 같은 동기 설명이 새 주제 도입과 함께 나오면 함께 인정한다. "
        "단, 실습 과제·연습 문제를 풀어보라는 지시 자체(다음 주제 선언 없이 과제만 제시), "
        "단순 진행 지시, UI·화면 안내는 전환 멘트가 아니므로 근거로 쓰지 않는다. 주요 주제 전환마다 "
        "안내가 있으면 4점 이상, 일부만 있으면 3점, 안내 없이 갑작스러운 이동이 "
        "반복되면 2점 이하."
    ),
    # ── C5 실습 및 적용 (담당: 김예슬) ───────────────────────────────────
