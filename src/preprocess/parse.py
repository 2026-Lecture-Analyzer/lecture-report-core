...
- 한 줄 파싱은 loader.parse_line 단일 파서를 공유한다(중복 제거).
...
from src.preprocess.loader import _FNAME_RE, _session_from_hour24, parse_line, to_24h

logger: logging.Logger = logging.getLogger(__name__)


def iter_raw_records(script_dir: Path | None = None) -> Iterator[dict]:
    ...
    for path in files:
        ...
        logger.debug("파싱 중: %s (%s)", path.name, date)
        with path.open(encoding="utf-8") as f:
            for line_no, raw in enumerate(f, start=1):
                ...
                if parsed is None:
                    ...
                    idx += 1
                    continue
                hh, mm, ss, speaker_id, text = parsed
                h24 = to_24h(hh)
                yield dict(
...
def write_raw_jsonl(
    out_path: Path, script_dir: Path | None = None
) -> dict[str, int | str]:
    """raw.jsonl 생성. 반환: 간단 통계(행수, 파일수, malformed수)."""
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    logger.debug("write_raw_jsonl 시작: out=%s", out_path)
    ...
    with out_path.open("w", encoding="utf-8") as w:
        for rec in iter_raw_records(script_dir):
            ...
            files.add(rec["file"])
    logger.debug("write_raw_jsonl 완료: %d건 (%d파일, malformed %d건)",
                 n, len(files), n_malformed)
