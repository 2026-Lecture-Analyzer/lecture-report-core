...
> **C5_check·engage 해결됨(2026-06-11, raw 직접 검증):** C5 과소를 "데이터 한계"로만 봤으나 실제 주범은 **C1과 동일한 refine 신호 삭제**였다. 02-02 raw vs clean cue: `되셨어요` 20→0 · `이해 가(죠)` 6→0 · `됐어요` 10→0 · `해보세요` 3→0 · `같이` 18→0 · `풀어` 4→0. raw에선 이해확인·참여유도가 수십 회인데 정제본엔 0개라 holistic이 "없음 1점" 판정. → **C5_check·C5_engage 도 raw cue 빈도(10분당) 메트릭으로 측정**(`config.C5_CHECK_CUES`/`C5_ENGAGE_CUES`, `metrics.py`). gold 02-02: check 1.4·engage 0.8/10분 → 둘 다 3점(gold 일치). C5_answer만 단일화자 한계 잔존.
