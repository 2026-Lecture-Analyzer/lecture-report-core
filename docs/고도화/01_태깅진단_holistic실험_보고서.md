...
| 입력 방식 | 항목 | 근거 |
|---|---|---|
| **raw 메트릭**(결정적, LLM 없음) | C1_repetition · C1_consistency · C1_completeness · C3_pace · **C5_check · C5_engage** | §8(C1)·§9-2(C5) — 정제본이 신호 지움 |
| **holistic**(전체원문 1패스 LLM) | 나머지 12 (C2 구조·C3 개념·C4 실습·C5_answer) | §7 — 사람 수준 |
...
| 하이브리드 (C1 raw) | 0.89 | 13/18 (72%) | -0.44 | 6 |
| **하이브리드 +C5 cue** | **0.61** | **15/18 (83%)** | **-0.17** | 6 |

→ **C5_check·engage 까지 raw cue 메트릭으로 돌리니 MAE 0.89 → 0.61, 편향 -0.44 → -0.17.** C5 둘 다 gold 3 정확 일치.

### 9-2. 잔존 오차 (±2, 3개만 남음 — 전부 기지 한계)
| 항목 | HYB | gold | 원인 | 후속 |
|---|---|---|---|---|
| C1_completeness | 5 | 3 | merged 블록이 끊김 뭉갬 | raw 발화(pre-merge) 미완결률 |
| C4_error | 2 | 4 | holistic 박함·SC 변동 | 프롬프트 가이드 보정 |
| C5_answer | 1 | 3 | 단일화자 데이터 한계(학생 발화 미분리) | 화자분리/프롬프트 재해석 |

> **C5_check·engage 해결됨(2026-06-11, raw 직접 검증):** C5 과소를 "데이터 한계"로만 봤으나 실제 주범은 **C1과 동일한 refine 신호 삭제**였다. 02-02 raw vs clean cue: `되셨어요` 20→0 · `이해 가(죠)` 6→0 · `됐어요` 10→0 · `해보세요` 3→0 · `같이` 18→0 · `풀어` 4→0. raw에선 이해확인·참여유도가 수십 회인데 정제본엔 0개라 holistic이 "없음 1점" 판정. → **raw cue 빈도(10분당) 메트릭으로 측정**(`config.C5_CHECK_CUES`/`C5_ENGAGE_CUES`, `metrics.py`). gold 02-02: check 1.4·engage 0.8/10분 → 둘 다 3점(gold 일치). C5_answer만 단일화자 한계 잔존.

> **정정(2026-06-11, raw 직접 검증):** C5 과소를 "데이터 한계"로만 봤으나, 실제 주범은 **C1과 동일한 refine 신호 삭제**다. 02-02 raw vs clean cue 잔존: `되셨어요` 20→0 · `이해 가(죠)` 6→0 · `됐어요` 10→0 · `해보세요` 3→0 · `같이` 18→0 · `풀어` 4→0. raw엔 이해확인·참여유도가 수십 회인데 정제본엔 0개라, holistic이 "없음 1점"으로 판정. → **C5_check(이해확인)·C5_engage(참여유도)도 raw cue 기반(메트릭)으로 측정**하면 복구된다(C5_answer만 단일화자 한계 잔존).
