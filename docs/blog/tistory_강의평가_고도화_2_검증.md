...
이번 작업에서 가장 크게 배운 것은 세 가지입니다. 첫째, 정답이 하나뿐인 평가 지표는 "정확한 것"과 "한 점에 맞춰진 것"을 구분할 수 없으므로, 좋아 보이는 숫자일수록 두 번째 정답으로 의심해 봐야 한다는 것입니다. 둘째, raw-holistic 같은 음성 결과는 실패가 아니라 설계의 경계를 알려주는 데이터이며, "왜 안 되는가"를 이해하면 오히려 기존 설계의 근거가 단단해집니다. 셋째, 데이터가 한 명의 강사로만 이루어져 있다는 한계는 끝까지 남으며, "변별력이 없다"와 "대상이 일관됐다"를 진짜로 가르려면 결국 다른 강사의 정답이 필요합니다. 화려한 기법보다 "이 숫자를 믿어도 되는가"를 끈질기게 따지는 과정이, 평가 시스템을 만들 때 가장 중요한 일이었습니다.
