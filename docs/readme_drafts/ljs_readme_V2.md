# 🎓 Lecture Analyzer — AI 강의 분석 리포트 생성기 (V2)

> STT로 추출한 강의 스크립트를 LLM으로 분석해, 강사의 **강의력을 다각도로 평가**하고 개선 인사이트를 담은 리포트를 자동 생성하는 시스템

NLP 과제 1 · AI 엔지니어 자연어처리과정 · 4인 1팀 · 4주 프로젝트

> **이 문서(readme_V2)는** V1 통합 설계를 바탕으로 **①~⑧ 전 파이프라인을 로컬 end-to-end 로 구현·검증**한 뒤, 이제 **폴더(모듈)별로 역할을 나눠 고도화**하는 단계의 기준 문서다. 진행 현황과 핵심 설계 변경(RAG 근거 탐색 방식)의 전후 결과는 바로 아래 「중간 점검 리포트」, V2 분담은 「팀 · 역할 분담」 참고.
...
### 1. 진행 현황 — ①~⑧ 전 파이프라인 로컬 end-to-end 완료
단일 강의 기준으로 **전처리 → 정제·청킹·태깅 → 4갈래 LLM 분석 → 항목가중 종합점수 → MD/PDF 리포트** 가 실데이터로 동작한다. 정제·임베딩을 **Upstage API 백엔드**로 분기해 GPU 없이 로컬에서 끝까지 실행할 수 있다.

| 단계 | 상태 | 단계 | 상태 |
|---|---|---|---|
| ①② 파싱·병합 | ✅ | ⑤ 임베딩 청킹·태깅 | ✅ (top-k) |
| ③ 용어집·개요 | ✅ | ⑥ 4갈래 분석 | ✅ (self-consistency) |
| ④ 정제(CTX/MAIN·개요) | ✅ | ⑦ 항목가중 스코어링 | ✅ |
| 단일화자·백엔드 분기 | ✅ | ⑧ MD/PDF 리포트 | ✅ |

검증: 스모크 6종(refine·chunk_embed·analyze·score·report) + 실데이터 2세션(2026-02-03 오전·오후) end-to-end + 항목별 사실 대조.

### 2. 핵심 설계 변경 — RAG 근거 탐색 방식 (전후 결과)

이번 단계에서 가장 크게 추론·개선한 부분은 **"평가 항목의 근거를 긴 전사에서 어떻게 검색하느냐"** 였다. 처음엔 키워드 사전 + 유사도 임계값으로 충분할 줄 알았으나, 구어체 데이터에서 문제가 드러났다.

**문제(변경 전):**
- **키워드 동음이의 오탐** — '실행'(구동 vs 실습), '오류'(API명 vs 오류대응), '따라'(조사 vs 따라하기) 등이 무관한 항목을 달았다.
- **임계값 전체긁기 → 과태깅** — "유사도 ≥ 임계인 청크 전부" 방식이라, 추상적인 항목(오류 대응)이 전체 청크의 절반에 붙었다.

**변경(설계 근거: RubricRAG=의미검색 표준, DAT=하이브리드):**
- **항목당 top-k 검색**으로 전환 — 항목마다 가장 관련된 상위 k개만 검색(`tagging.py`). dense 임베딩이 주 신호, 키워드는 **가산점·임계 완화(floor 구제)** 보조. **동음이의 키워드는 사전에서 제거**.
- **검색은 재현율, 판정은 정밀도** — ⑤ 태깅은 후보만 추리고, 정밀 판정은 ⑥ LLM이 청크를 읽고 한다(잘못 태깅돼도 LLM이 거름).

**결과(전후 비교 · 2026-02-03 오전, 95청크 기준):**

| 항목 | 변경 전(키워드·임계값) | 변경 후(top-k 하이브리드) |
|---|---|---|
| 태깅 방식 | 임계값 넘으면 전부 | **항목당 top-k(=5)** |
| `C4_error` 태그 수 | 49 (전체의 52%) | **10** (과태깅 해소) |
| 총 태그 수 | 192 | **85** |
| 동음이의 cue 오탐 | 실행 14·오류 11·따라 6 … | **0** (사전 정리) |
| 진짜 비유("…처럼") | top-k 탈락 위험 | **cue floor 구제로 복원** |
| `C3_definition` 실측 점수 | **1점 "정의 없음"**(오판) | **5점 "우수"**(IO 정의 인식) |
| 종합 강의력 점수(오전) | 33.9 | **42.2** |

> ⑤ 임베딩이 한 항목에 무관한 청크를 잘못 태깅해도, ⑥ LLM이 "해당 없음"으로 낮은 점수를 줘 **검색-판정 분업이 실증**됐다. 임베딩만으로 약한 상호작용 항목(C5)은 세션별로 정확히 차등됐다(강의형 오전 8.3 vs 실습형 오후 25.0).

### 3. 판정 안정화 — false negative 수정

`C3_definition`이 같은 입력에도 실행마다 흔들리고(1점↔3점), 명백한 정의를 "없음"으로 오판하는 false negative가 있었다. 셋으로 보완했다.
- **노이즈 감소** — cue 가산점을 키워 진짜 근거가 일반 청크보다 위에 랭크되게.
- **프롬프트 보강** — "A는 B를 의미합니다" 같은 평이한 서술도 정의로 인정, 명백한 근거 있으면 "없음" 금지.
- **self-consistency** — 항목당 여러 번 채점 후 다수결(`--self-consistency N`). LLM-Rubric의 보정 정신.
→ 1점으로 오판되던 항목이 5점으로 바로잡혔고, 실행 간 점수 흔들림이 줄었다.

### 4. 한계 · 다음 (V2 고도화로 연결)
- LLM 판정 정확도 100% 아님 → 더 많은 강의 + 사람 스팟체크로 정량화 필요.
- 지표 임계(pace/filler)는 **잠정값** → 2차 EDA 캘리브레이션.
- 전체 15강의 배치(강사 비교·주차 추이), 대시보드, raw_ref 정밀 정렬은 미완. → **§팀 · 역할 분담**의 모듈별 고도화로 진행.
...
├── readme_V2.md                           # (현재 파일) V2 — 모듈별 분담·중간 점검 리포트
├── readme_V1.md                           # V1 통합 설계 스펙(아카이브)
...
## 🔍 분석 엔진(⑥) V2 고도화 — 검증 가능한 채점 엔진 (이지선)

> readme_V2 의 「⚙️ ⑥ 분석 엔진 상세 로직」 절 뒤에 이어 붙이는 작업 기록.

### 무엇을 바꿨나 (한눈에)

| # | 영역 | 파일 | 목적 |
|---|---|---|---|
| 1 | 항목 평가 기준 구체화 | `checklist.py` | LLM 오판(단어만 보고 고득점) 감소 |
| 2 | 항목별 가이드 + 교차검증 프롬프트 | `prompts.py` | 항목 특화 채점, 태그 0개 false negative 방지 |
| 3 | 전역형 규칙 채점 | `metrics.py` | 전역 항목(C1) 점수에 지표 닻(anchor) 추가 |
| 4 | 분석 엔진 안전장치 5종 | `engine.py` | 근거 검증·점수 추적·교차검증·문맥확장·라우팅 로그 |
| 5 | 검수 도구 2종(신규) | `scripts/inspect_*` | context·tagging 품질 사후 점검(파이프라인 밖) |
| 6 | 데이터 계약 갱신 | `docs/SCHEMA.md` | `analysis.jsonl` 에 `scoring_trace`·확장 `routing` 반영 |
| 7 | 러너 표시 정확화 | `run_analyze_local.py` | 교차검증 호출 추정·N/A·강등 집계 표시 |

### 1. 항목 평가 기준 구체화 — `checklist.py`
PDF 기준을 그대로 옮긴 `description` 은 LLM 이 "단어만 보고" 점수를 주기 쉬웠다. 8개 항목
(`C2_order`·`C3_definition`·`C3_analogy`·`C3_prerequisite`·`C4_error`·`C5_check`·`C5_engage`)의
`description` 을 **무엇을 인정하고 무엇을 감점하는지**까지 명시하도록 확장했다.
- 예) `C4_error`: "오류를 **언급만** 했는지, **원인+해결**까지 안내했는지 구분. 단어만 등장하면 2점 이하."
- 예) `C5_engage`: "강사 혼자 시연하며 '해볼게요'는 참여 유도 ❌. 청자 행동 요청('해보세요')만 인정."
- `key`/`category`/`eval_type` 등 **스키마 필드는 그대로** — 채점 기준 텍스트만 강화(계약 영향 없음).

### 2. 항목별 가이드 + 교차검증 프롬프트 — `prompts.py`
- **`ITEM_GUIDES`**: 공통 `_JUDGE_SYS` 위에 얹는 항목 특화 채점 기준 8종. `judge_prompt()` 가
  `[항목별 추가 기준]` 으로 자동 주입.
- **`cross_check_prompt()`**(신규): 태그 0개일 때 "정말 부재인지"를 샘플로 재확인. `found` 불리언 응답.
- **문맥 인용 문구 통일**: `[참고 문맥(채점 대상 아님)]` → "문맥 안에 실제 근거가 있으면 evidence 로 인용 가능"
  으로 바꿔 engine·검수 스크립트 동작과 일치시킴.

### 3. 전역형 규칙 채점 — `metrics.py`
전역 항목(🔴 global)은 LLM 단독이라 흔들리기 쉬워, 지표로 채점 가능한 두 항목에 규칙 점수(anchor)를
추가(`score_global_metric_item()`).
- `C1_consistency` ← `honorific_ratio`: 0.9+ →5 / 0.7~0.9 →3 / 0.7- →2.
- `C1_completeness` ← `incomplete_ratio`: 0.1- →5 / 0.1~0.25 →4 / 0.25+ →2.
- 지표 없으면(`None`) LLM 단독으로 fallback.

### 4. 분석 엔진 안전장치 5종 — `engine.py`
1. **근거 없는 고득점 방지**(`_validate_evidence`): `score≥4` 인데 `evidence` 비면 **2점·"근거 부족"** 강등,
   `scoring_trace.evidence_adjusted=true` 기록(원점수 보존). 지표형은 제외.
2. **self-consistency trace**(`_aggregate`): `{raw_scores, final_score, agreement}` 로 확장 — 점수 안정성 가시화.
3. **태그 0개 교차검증**(`_cross_check_negative`): 부재(1점) 단정 전 샘플 재확인.
   **위치형은 구간 한정** — intro→head, outro→tail, local→전체 균등.
4. **비대칭 문맥확장**(`CONTEXT_POLICY`+`_neighbors`): 항목별 `before/after`.
   `C4_error`=1/3(해결 뒤), `C5_answer`=2/2, `C3_definition`=1/1.
5. **라우팅 로그 확장**: `candidate_chunk_ids`·`context_chunk_ids`·`cross_checked`·`rule_mixed` 추가.
- 전역형 **규칙·LLM 혼합**: 평균 `int((a+b)/2+0.5)`(짝수 반올림 회피), 혼합 시 `scoring_trace` 갱신.
- **merged 부재 시 metric 항목 `score=null`·"N/A"** — pace·filler 0 왜곡 방지.

### 5. 검수 도구 2종 (신규·파이프라인 밖) — `scripts/inspect_*`
LLM·임베딩 재호출 없이 산출물만 읽어 사람이 검수.
- **`inspect_context_quality.py`**: routing 의 candidate/context 를 prev/next 로 출력 + 범위 밖 인용 탐지.
- **`inspect_tagging_quality.py`**: `eval_tags` 항목별 score 정렬 + seed_keyword 있는데 태그 0인 false-negative 후보 표시
  (embedding 재계산 ❌, 태깅 결과 기준).
- 파이프라인에 넣지 않고 **별도 실행**(검수·디버깅 전용).

### 6. 데이터 계약 갱신 — `docs/SCHEMA.md`
`analysis.jsonl` 필드 증가 → **계약 영향 있음**(SCHEMA 먼저 갱신·공유 후 코드).
- 신규 `scoring_trace`(+`evidence_adjusted`/`rule_score`), `routing` 확장, `verdict` 에 `근거 부족`·`N/A`, `score` `null` 가능.
- **P3 호환 확인 완료**: `scoring.py` 가 `score=None` 을 `n_na` 로 집계·가중 제외 — 수정 불필요.

### 7. 러너 표시 정확화 — `run_analyze_local.py`
동작 변화 없이 표시만: 실행 전 호출 추정에 **태그0 교차검증 호출** 가산, 완료 요약에 **N/A·근거강등 수** 추가.

### 검증
stub 백엔드 통합 테스트 전부 통과 — 근거 강등(+trace 갱신)·agreement·비대칭 문맥확장·교차검증 양방향·
전역 규칙 혼합(반올림)·N/A·위치 샘플(head/tail). 전 파일 구문 검증 완료.

> 커밋: `analysis/이지선/v2`. checklist·prompts·metrics·engine 은 `feature-analysis:`,
> inspect 스크립트 별도 커밋, SCHEMA 는 `docs:` 로 먼저 공유. PR 본문 `closes #<이슈>`.
...
> V1에서 ①~⑧ end-to-end가 동작하므로, V2는 **폴더(모듈) 단위로 담당을 나눠 각자 고도화**한다.
> 단계 간 인터페이스(JSONL 계약)는 **[docs/SCHEMA.md](docs/SCHEMA.md)** — 이것만 지키면 4명이 병렬로 작업한다.

| 담당 | 폴더 | 입력 → 산출물 | 이름 |
|---|---|---|---|
| **정제·청킹·태깅** | `src/refine/` | `merged.jsonl` → `clean.jsonl`, `chunks.jsonl` | **김예슬** |
| **분석 엔진(4갈래)** | `src/analyze/` | `chunks.jsonl` → `analysis.jsonl` | **이지선** |
| **스코어링·검증** | `src/scoring/` | `analysis.jsonl` → `scores.json` | **정찬희** |
| **리포트·PDF** | `src/report/` | `scores.json` → 리포트(MD/PDF) | **박채린** |

### 모듈별 고도화 목표 (V2)

**`src/refine/` — 김예슬 (정제·청킹·태깅)**
- 항목당 top-k 검색의 `floor`/`TOP_K`/`cue 가산점` 캘리브레이션, **항목 임베딩을 exemplar 문장으로** 강화(RubricRAG식 변별력↑).
- 정제 품질(군더더기 제거·STT 교정), fallback 비율↓, 개요(아웃라인) LLM 도입 효과 실측, 청크 `raw_ref` 정밀 정렬.

**`src/analyze/` — 이지선 (분석 엔진)**
- self-consistency 표본수·온도 튜닝, 🟠 문맥확장(`_neighbors`) 정밀화, 🔴 전역형 압축뷰 개선, 🔵 지표 임계 반영.
- 유형별 프롬프트 정교화(few-shot), false negative/positive 패턴 분석, (라벨 확보 시) calibration.

**`src/scoring/` — 정찬희 (스코어링·검증)**
- 가중치 확정, **강사/과목별 비교 집계**, 주차별 시계열 추이, `evaluate.py`로 메타데이터(정답) 기반 정확도 검증(⚠️ 메타는 검증 전용·input 금지).

**`src/report/` — 박채린 (리포트·PDF)**
- PDF 레이아웃 고도화(카테고리 레이더 차트·표 정렬·페이지), 강사 비교/추이 리포트, 대시보드(Streamlit), 폰트·다국어 대응.

> 공통 규칙: 모듈 간 계약은 `docs/SCHEMA.md` 의 JSONL을 따른다. 필드를 바꿔야 하면 **먼저 SCHEMA를 고치고 공유** → 그 다음 코드.
